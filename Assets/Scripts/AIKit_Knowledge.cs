    ...
    public class KnowledgeModule {
            ...
            foreach(SemanticWebNode f in possibleFroms) {
                foreach( SemanticWebNode t in possibleTos) {
                    ...
                    foreach (SemanticWebEdge e in antiMatchingEdges) {
                        nocount++;
                        recentNo += Mathf.Exp(AIKit_World.Now().val() - e.why.utterance.val()); 
                    }
        ...
        public bool isTrue(SemSentence s, out string console, bool rr){
            ...
            switch (GetLogicClass(root)) 
            {
                ...
                case LogicClass.Universal:
                    console+="A universal subject -- all nodes must matchn (or the universal itself is true?)\n";

                    //check if atomically true first
                    SemanticWebNode atomicAnyRootNode = lexicalMemory.GetOrInsert(root);
                    string addtlAtomic;
                    bool anyNodeIsTrue = BranchIsTrue(atomicAnyRootNode, branch, out addtlAtomic, rr);
                    console += addtlAtomic;
                    if (anyNodeIsTrue)
                    {
                        console += "Any is atomically true!";
                        return true;
                    }
        ...
        public Stack<SemSentence> PlanTo(SemSentence goal)
        {
            ...
            Debug.LogError("Planning to " + goal.ToString() + " finished!");
